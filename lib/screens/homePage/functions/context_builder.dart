...
class contextBuilderPage extends StatefulWidget {
  @override
  _contextBuilderPageState createState() => _contextBuilderPageState();
}

class _contextBuilderPageState extends State<contextBuilderPage> {
            ...
            Center(
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  validatePPTTopic(topic, validationPrompt); // Updated function call
                },
                child: const Text(
  ...
  void validatePPTTopic(String topic, String validationPrompt) async {
    if (topic.isEmpty) {
      showErrorDialog('Please enter a topic.');
      return;
    }
    ...
    try {
      ...
      if (response.text != null && response.text!.isNotEmpty) {
        if (response.text!.toLowerCase().startsWith('yes')) {
          String prompt = "Act as a qualified CBSE school Teacher from India of Class $selectedClassSubject who specializes in teaching $selectedClassSubject and generate a Context Builder that introduces the topic to students of the age ($selectedClassSubject + 4) for the topic $topic";

          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => geminiScreen(
                prompt: prompt, contentType: 'PPT',
              ),
            ),
          );
        } else {
